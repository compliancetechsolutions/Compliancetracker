 ...
 * 
 * import java.security.Key; import java.util.Date; import java.util.List;
 * 
 ...
 * 1000 * 60 * 30)) .signWith(key) .compact(); }
 * 
 * // ========================= // REFRESH TOKEN // =========================
 ...
 * 
 * if (roles instanceof List<?>) { return ((List<?>) roles) .stream()
 ...
 * 
 * try { return validate(token).getExpiration().before(new Date()); } catch
 * (Exception e) { return true; } }
 * 
 * // ========================= // OPTIONAL: ROLE → SCOPES //
 * ========================= public List<String> getScopes(String role) { return
 * switch (role) { case "ADMIN" -> List.of("system:write", "user:manage",
 * "activity:all"); case "INVESTOR" -> List.of("report:read", "dashboard:view");
 * case "INITIATOR" -> List.of("activity:update", "tracker:read"); default ->
 * List.of(); }; } }
 */